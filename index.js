...
function getUsersFromDatabase(array) {
  ...
  return Array.from(array, (elem) => {elem.name = String(elem.name).toUpperCase(); return elem;});
}
...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    console.log(`Вхідний масив порожній`);
    return [];    
  }
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    let fIdx = arr.indexOf(element);
    let lIdx = arr.lastIndexOf(element);
    result.push(fIdx);
    if (fIdx !== lIdx) {
      result.push(lIdx);
    }    
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (!Array.isArray(arr) || start > end || start >= arr.length || end >= arr.length) {
    return false;
  }
...
function customAt(arr, index) {
  ...
  switch (typeof elem) {
    case 'string' : str = `Елемент є рядком`; break;
    case 'number' : str = `Елемент є числом`; break;
    case 'object' : str = `Елемент є обєктом`; break;
    default : str = 'Щось не зрозуміле :('; break;
  }
...
function customIncludes(arr, element) {
  ...
  if (arr.includes(element)) {
    console.log(arr.filter(elem => elem === element).length);
    return true;
  }
}
